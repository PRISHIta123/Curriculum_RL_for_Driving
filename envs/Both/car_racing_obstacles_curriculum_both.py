...
class CarRacingObstaclesCurriculumBoth(gym.Env, EzPickle):
    metadata = {
        "render.modes": ["human", "rgb_array", "state_pixels"],
        "video.frames_per_second": FPS,
    }

    def __init__(self, ranges, verbose=1):
    ...
    def reset(self):
        ...
        if self.gen>self.ranges[0] and self.gen<=self.ranges[1]:
            TRACK_TURN_RATE=0.41
            OBSTACLE_PROB=0.07
        elif self.gen>self.ranges[1] and self.gen<=self.ranges[2]:
            TRACK_TURN_RATE=0.51
            OBSTACLE_PROB=0.09
        elif self.gen>self.ranges[2]:
            TRACK_TURN_RATE=0.61
            OBSTACLE_PROB=0.11
...
if __name__ == "__main__":
    ...
    env = CarRacingObstaclesCurriculumBoth()
    env.render()
